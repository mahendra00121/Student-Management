...
export default function EditStudentPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            ...
            alert("Failed to update student. Please check console for errors.");
        }
    }

    const selectedClass = form.watch("class");
    const availableSections = classes.find(c => c.name === selectedClass)?.sections || [];

    if (!getStudent(id)) {
        return <div className="p-8 text-center font-bold">Loading student data...</div>
    }
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value} disabled={!selectedClass}>
                                                <FormControl>
                                                    <SelectTrigger>
