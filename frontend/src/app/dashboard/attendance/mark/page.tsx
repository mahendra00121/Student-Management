...
export default function MarkAttendancePage() {
    ...
    const handleLoadStudents = async () => {
        ...
        try {
            ...
            console.log("Filtered students:", classStudents.length);
            setFilteredStudents(classStudents);

            const existingRecords = await getAttendanceByDateAndClass(selectedDate, selectedClass, selectedSection);
            ...
            console.error("Error in handleLoadStudents:", error);
            alert("Failed to load registry. Please check console.");
        } finally {
            setIsFetching(false);
        }
    };
    ...
    const handleSaveAttendance = async () => {
        if (filteredStudents.length === 0) {
            alert("No students loaded to save attendance.");
            return;
        }
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center gap-4 no-print">
                ...
                <span className="text-xs font-bold text-muted-foreground">Return to Registry</span>
            </div>
                    ...
                    <Card className="border shadow-2xl shadow-muted/50 rounded-3xl overflow-hidden">
                        <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/20 pb-6 pt-6">
                            ...
                            <Button
                                ...
                                disabled={isSaving}
                                className="gap-2 font-bold rounded-xl shadow-xl shadow-primary/20 bg-primary px-6 h-11"
                            >
                                {isSaving ? (
                                    <>
                                        <div className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2" />
